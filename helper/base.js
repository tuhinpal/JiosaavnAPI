module.exports = {
  ...
  albumsearchFromSTRING: (query) =>
    `https://www.jiosaavn.com/api.php?__call=autocomplete.get&_format=json&_marker=0&cc=in&includeMetaTags=1&query=${query.split(' ').join('+')}`,
  songFromID: (id) =>
  ...
  songsearchFromSTRING: (query) =>
    `https://www.jiosaavn.com/api.php?p=1&_format=json&_marker=0&api_version=4&ctx=wap6dot0&n=10&__call=search.getResults&q=${query.split(' ').join('+')}`,
    lyricsFromID: (id) =>
